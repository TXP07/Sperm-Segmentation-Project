...
input_path = f"imagec/yuan/{I}.jpg"  # Path to the input image
output_path = "image/yuan/new.jpg" # Path to save the resized image
...
# 单一文件路径
image_path = 'image/yuan/new.jpg' # 请替换为你的图片文件路径
...
# 指定单一图像路径
image_path = 'image/yuan/new2.jpg'  # 替换为你的图片路径
news_path = f'image/{I}.jpg'
# 调用处理函数
...
# Set your parameters here
input_path = f"image/{I}.jpg"  # Path to the input image
output_path = f"image/{I}.jpg" # Path to save the resized image
...
image_path = f'image/yuan/{I}.jpg' # 根据您的文件路径进行调整
image = cv2.imread(image_path)
...
image_path = f'image/yuan/{I}.jpg'  # 确保路径和变量名正确
...
selected_tails10 = find_matching_tails(longest_lines_info, all_endpoints, 10)#不能再大于27
selected_tails20 = find_matching_tails(longest_lines_info, all_endpoints, 20)#不能再大于27
selected_tails40 = find_matching_tails(longest_lines_info, all_endpoints, 40)#不能再大于27
selected_tails50 = find_matching_tails(longest_lines_info, all_endpoints, 50)#不能再大于27
selected_tails60 = find_matching_tails(longest_lines_info, all_endpoints, 60)#不能再大于27
...
for head_layer, tail_layers in selected_tails10.items():
    ...
    if min_angle_diff > 40:  # 可修改的角度阈值(不可大于62)
        final_selection10[head_layer] = None
    else:
        final_selection10[head_layer] = selected_tail

for head_layer, tail_layers in selected_tails20.items():
    head = heads[head_layer]
    min_angle_diff = float('inf')
    selected_tail = None
    ...
    # Check if the minimum angle difference exceeds 100 degrees
    if min_angle_diff > 40:  # 可修改的角度阈值(不可大于62)
        final_selection20[head_layer] = None
    else:
        final_selection20[head_layer] = selected_tail
...
for head_layer, tail_layers in selected_tails40.items():
    head = heads[head_layer]
    min_angle_diff = float('inf')
    selected_tail = None
    ...
    # Check if the minimum angle difference exceeds 100 degrees
    if min_angle_diff > 40:  # 可修改的角度阈值(不可大于62)
        final_selection40[head_layer] = None
    else:
        final_selection40[head_layer] = selected_tail





for head_layer, tail_layers in selected_tails50.items():
    head = heads[head_layer]
    min_angle_diff = float('inf')
    selected_tail = None
    ...
    # Check if the minimum angle difference exceeds 100 degrees
    if min_angle_diff > 40:  # 可修改的角度阈值(不可大于62)
        final_selection50[head_layer] = None
    else:
        final_selection50[head_layer] = selected_tail




for head_layer, tail_layers in selected_tails60.items():
    head = heads[head_layer]
    min_angle_diff = float('inf')
    selected_tail = None
    ...
    # Check if the minimum angle difference exceeds 100 degrees
    if min_angle_diff > 40:  # 可修改的角度阈值(不可大于62)
        final_selection60[head_layer] = None
    else:
        final_selection60[head_layer] = selected_tail
...
while True:
    ...
    tail_to_heads = {}
    for head, tail in final_selection10.items():
        if tail is not None:
    ...
    for tail, conflicting_heads in conflict_tails.items():
        ...
        for head in conflicting_heads:
            if head != selected_head:
                ...
                final_selection10[head] = alternative_tails[0] if alternative_tails else None
...
while True:
    # Find all heads assigned to each tail
    tail_to_heads = {}
    for head, tail in final_selection20.items():
        if tail is not None:
            tail_to_heads.setdefault(tail, []).append(head)
    ...
    for tail, conflicting_heads in conflict_tails.items():
        ...
        for head in conflicting_heads:
            if head != selected_head:
                alternative_tails = [t for t in selected_tails[head] if t != tail and t not in tail_to_heads]
                final_selection20[head] = alternative_tails[0] if alternative_tails else None
...
while True:
    # Find all heads assigned to each tail
    tail_to_heads = {}
    for head, tail in final_selection40.items():
        if tail is not None:
            tail_to_heads.setdefault(tail, []).append(head)
    ...
    for tail, conflicting_heads in conflict_tails.items():
        ...
        for head in conflicting_heads:
            if head != selected_head:
                alternative_tails = [t for t in selected_tails[head] if t != tail and t not in tail_to_heads]
                final_selection40[head] = alternative_tails[0] if alternative_tails else None               
...
while True:
    # Find all heads assigned to each tail
    tail_to_heads = {}
    for head, tail in final_selection50.items():
        if tail is not None:
            tail_to_heads.setdefault(tail, []).append(head)
    ...
    for tail, conflicting_heads in conflict_tails.items():
        ...
        for head in conflicting_heads:
            if head != selected_head:
                alternative_tails = [t for t in selected_tails[head] if t != tail and t not in tail_to_heads]
                final_selection50[head] = alternative_tails[0] if alternative_tails else None                
...
while True:
    # Find all heads assigned to each tail
    tail_to_heads = {}
    for head, tail in final_selection60.items():
        if tail is not None:
            tail_to_heads.setdefault(tail, []).append(head)
    ...
    for tail, conflicting_heads in conflict_tails.items():
        ...
        for head in conflicting_heads:
            if head != selected_head:
                alternative_tails = [t for t in selected_tails[head] if t != tail and t not in tail_to_heads]
                final_selection60[head] = alternative_tails[0] if alternative_tails else None
...
# 过滤掉 None 值
final_selection_filtered10 = {k: v for k, v in final_selection10.items() if v is not None}
final_selection_filtered20 = {k: v for k, v in final_selection20.items() if v is not None}
final_selection_filtered = {k: v for k, v in final_selection3.items() if v is not None}
final_selection_filtered40 = {k: v for k, v in final_selection40.items() if v is not None}
final_selection_filtered50 = {k: v for k, v in final_selection50.items() if v is not None}
final_selection_filtered60 = {k: v for k, v in final_selection60.items() if v is not None}
...
# 初始化两个数组来分别存储头部和尾部的原始掩码
original_head_masks10 = []
original_tail_masks10 = []

# 遍历 final_selection_filtered 字典
for i, (head_layer, tail_layer) in enumerate(final_selection_filtered10.items()):
    head_mask = head_masks[head_layer]
    tail_mask = segmentations[tail_layer]

    # 将原始的头部和尾部掩码添加到相应的数组中
    original_head_masks10.append(head_mask)
    original_tail_masks10.append(tail_mask)

    # 重叠掩码
    combined_mask10 = np.maximum(head_mask, tail_mask)

    # 将组合掩码存储在字典中
    combined_masks10[f'Head_{head_layer}_Tail_{tail_layer}'] = combined_mask10



# 初始化两个数组来分别存储头部和尾部的原始掩码
original_head_masks20 = []
original_tail_masks20 = []

# 遍历 final_selection_filtered 字典
for i, (head_layer, tail_layer) in enumerate(final_selection_filtered20.items()):
    head_mask = head_masks[head_layer]
    tail_mask = segmentations[tail_layer]

    # 将原始的头部和尾部掩码添加到相应的数组中
    original_head_masks20.append(head_mask)
    original_tail_masks20.append(tail_mask)

    # 重叠掩码
    combined_mask20 = np.maximum(head_mask, tail_mask)

    # 将组合掩码存储在字典中
    combined_masks20[f'Head_{head_layer}_Tail_{tail_layer}'] = combined_mask20



# 初始化两个数组来分别存储头部和尾部的原始掩码
original_head_masks40 = []
original_tail_masks40 = []

# 遍历 final_selection_filtered 字典
for i, (head_layer, tail_layer) in enumerate(final_selection_filtered40.items()):
    head_mask = head_masks[head_layer]
    tail_mask = segmentations[tail_layer]

    # 将原始的头部和尾部掩码添加到相应的数组中
    original_head_masks40.append(head_mask)
    original_tail_masks40.append(tail_mask)

    # 重叠掩码
    combined_mask40 = np.maximum(head_mask, tail_mask)

    # 将组合掩码存储在字典中
    combined_masks40[f'Head_{head_layer}_Tail_{tail_layer}'] = combined_mask40
...
# 初始化两个数组来分别存储头部和尾部的原始掩码
original_head_masks50 = []
original_tail_masks50 = []

# 遍历 final_selection_filtered 字典
for i, (head_layer, tail_layer) in enumerate(final_selection_filtered50.items()):
    head_mask = head_masks[head_layer]
    tail_mask = segmentations[tail_layer]

    # 将原始的头部和尾部掩码添加到相应的数组中
    original_head_masks50.append(head_mask)
    original_tail_masks50.append(tail_mask)

    # 重叠掩码
    combined_mask50 = np.maximum(head_mask, tail_mask)

    # 将组合掩码存储在字典中
    combined_masks50[f'Head_{head_layer}_Tail_{tail_layer}'] = combined_mask50




# 初始化两个数组来分别存储头部和尾部的原始掩码
original_head_masks60 = []
original_tail_masks60 = []

# 遍历 final_selection_filtered 字典
for i, (head_layer, tail_layer) in enumerate(final_selection_filtered60.items()):
    head_mask = head_masks[head_layer]
    tail_mask = segmentations[tail_layer]

    # 将原始的头部和尾部掩码添加到相应的数组中
    original_head_masks60.append(head_mask)
    original_tail_masks60.append(tail_mask)

    # 重叠掩码
    combined_mask60 = np.maximum(head_mask, tail_mask)

    # 将组合掩码存储在字典中
    combined_masks60[f'Head_{head_layer}_Tail_{tail_layer}'] = combined_mask60
...
np.save('combined_masks10.npy', combined_masks10)
np.save('combined_masks20.npy', combined_masks20)
np.save('combined_masks40.npy', combined_masks40)
np.save('combined_masks50.npy', combined_masks50)
np.save('combined_masks60.npy', combined_masks60)
...
# Load the mask file
file_path = 'combined_masks10.npy'
masks = np.load(file_path, allow_pickle=True)
content = masks.item()
...
# Save the updated masks to a new file
updated_file_path = 'combined_masks10.npy'
np.save(updated_file_path, updated_content)



# Load the mask file
file_path = 'combined_masks20.npy'
masks = np.load(file_path, allow_pickle=True)
content = masks.item()
...
# Save the updated masks to a new file
updated_file_path = 'combined_masks20.npy'
np.save(updated_file_path, updated_content)



# Load the mask file
file_path = 'combined_masks40.npy'
masks = np.load(file_path, allow_pickle=True)
content = masks.item()
...
# Save the updated masks to a new file
updated_file_path = 'combined_masks40.npy'
np.save(updated_file_path, updated_content)



# Load the mask file
file_path = 'combined_masks50.npy'
masks = np.load(file_path, allow_pickle=True)
content = masks.item()
...
# Save the updated masks to a new file
updated_file_path = 'combined_masks50.npy'
np.save(updated_file_path, updated_content)
...
# Load the mask file
file_path = 'combined_masks60.npy'
masks = np.load(file_path, allow_pickle=True)
content = masks.item()
...
# Save the updated masks to a new file
updated_file_path = 'combined_masks60.npy'
np.save(updated_file_path, updated_content)
...
# Load the numpy array
data = np.load('combined_masks10.npy', allow_pickle=True)
...
np.save("combined_masks10.npy",Last)
...
# Load the numpy array
data = np.load('combined_masks20.npy', allow_pickle=True)
...
np.save("combined_masks20.npy",Last)
...
# Load the numpy array
data = np.load('combined_masks40.npy', allow_pickle=True)
...
np.save("combined_masks40.npy",Last)
...
# Load the numpy array
data = np.load('combined_masks50.npy', allow_pickle=True)
...
np.save("combined_masks50.npy",Last)
...
# Load the numpy array
data = np.load('combined_masks60.npy', allow_pickle=True)
...
np.save("combined_masks60.npy",Last)
...
npy_file_path10 = 'combined_masks10.npy'
npy_file_path20 = 'combined_masks20.npy'
npy_file_path40 = 'combined_masks40.npy'
npy_file_path50 = 'combined_masks50.npy'
npy_file_path60 = 'combined_masks60.npy'
...
masks10 = np.load(npy_file_path10, allow_pickle=True)
masks20 = np.load(npy_file_path20, allow_pickle=True)
masks40 = np.load(npy_file_path40, allow_pickle=True)
masks50 = np.load(npy_file_path50, allow_pickle=True)
masks60 = np.load(npy_file_path60, allow_pickle=True)
...
for index, mask in enumerate(masks10):
...
json_output_path = f'10/{I}.json'  # Replace 'I' with appropriate file name logic
...
for index, mask in enumerate(masks20):
    polygons = mask_to_polygon_points(mask)
    for polygon in polygons:
        shape_data = {
            "label": "sperm",
...
# Assuming 'I' is defined earlier in your code or you replace it with appropriate logic to generate file names
json_output_path = f'20/{I}.json'  # Replace 'I' with appropriate file name logic
with open(json_output_path, 'w') as json_file:
    json.dump(json_output, json_file, indent=4)
...
for index, mask in enumerate(masks40):
    polygons = mask_to_polygon_points(mask)
    for polygon in polygons:
        shape_data = {
            "label": "sperm",
...
# Assuming 'I' is defined earlier in your code or you replace it with appropriate logic to generate file names
json_output_path = f'40/{I}.json'  # Replace 'I' with appropriate file name logic
with open(json_output_path, 'w') as json_file:
    json.dump(json_output, json_file, indent=4)
...
for index, mask in enumerate(masks50):
    polygons = mask_to_polygon_points(mask)
    for polygon in polygons:
        shape_data = {
            "label": "sperm",
...
# Assuming 'I' is defined earlier in your code or you replace it with appropriate logic to generate file names
json_output_path = f'50/{I}.json'  # Replace 'I' with appropriate file name logic
with open(json_output_path, 'w') as json_file:
    json.dump(json_output, json_file, indent=4)
...
for index, mask in enumerate(masks60):
    polygons = mask_to_polygon_points(mask)
    for polygon in polygons:
        shape_data = {
            "label": "sperm",
...
# Assuming 'I' is defined earlier in your code or you replace it with appropriate logic to generate file names
json_output_path = f'60/{I}.json'  # Replace 'I' with appropriate file name logic
with open(json_output_path, 'w') as json_file:
    json.dump(json_output, json_file, indent=4)
...
# Assuming 'I' is defined earlier in your code or you replace it with appropriate logic to generate file names
json_output_path = f'image/yuan/{I}.json'  # Replace 'I' with appropriate file name logic
with open(json_output_path, 'w') as json_file:
    json.dump(json_output, json_file, indent=4)
...
# 定义文件路径
json_file_path = f'image/yuan/{I}.json'
